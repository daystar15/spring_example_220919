	...
	<script>
		$(document).ready(function() {
			...
			$("#nameCheckBtn").on("click", function() {
				...
				// 이름이 입력 되었는지 확인 - validation
				if (name.length < 1) {
					$("#nameStatusArea").append('<span class="text-danger">이름이 비어있습니다.</span>'); // append 동적으로 태그를 넣는다.
					return; // 빠져나온다(잊지말기)
				}
				...
				$.ajax({
					...
					, error:function(e) { // 원래 파라미터는 3개임
						alert("실패" + e) // 서버에 요청도 못했다, AJAX 자체가 실패
					}
				});
			});
		});
