...
<body>
	...
	<c:set var="number1" value="100" /> <%-- value=""이 방식으로 사용하자 --%>
	number1: ${number1}<br>
	...
	<h1>3. 조건문 (c:if)</h1>
	<c:if test="${number1 > 50}"> <%-- 참이되면 수행 --%>
		number1은 50보다 크다.<br>
	</c:if>
	<c:if test="${number1 == 100}">
		number1은 100이다.<br>
	</c:if>
	<c:if test="${number1 eq 100}"> <%-- eq는 ==와 같은 의미 --%>
		number1은 100이다.<br>
	</c:if>
	<c:if test="${number1 ne 222}"> <%-- ne와 !=는 같은 의미 --%>
		number1은 222가 아니다.<br>
	</c:if>
	<c:if test="${not empty number1}"> <%-- not empty는 비어있지 않다. --%>
		number1은 비어있지 않다.
	</c:if>
</body>
</html>